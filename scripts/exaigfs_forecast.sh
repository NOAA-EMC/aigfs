...
for fh in $( seq -f "%03g" 0 6 384 ); do
  file=$COMOUT/aigfs.${cycle}.pres.0p25.f${fh}.grib2
  cpfs mlgfs.${cycle}.pres.0p25.f${fh}.grib2 ${file}
  cpfs mlgfs.${cycle}.pres.0p25.f${fh}.grib2.idx ${file}.idx

  if [ "$SENDDBN" = "YES" ]; then 
    $DBNROOT/bin/dbn_alert MODEL AIGFS_GB2 $job ${file}
    $DBNROOT/bin/dbn_alert MODEL AIGFS_GB2_IDX $job ${file}.idx
  fi

  file=$COMOUT/aigfs.${cycle}.sfc.0p25.f${fh}.grib2
